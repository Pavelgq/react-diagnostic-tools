---
title: withExecutionTiming
description: A decorator that measures and logs how long any function takes to run.
---

import { WithExecutionTimingDemo } from '@/components/demos/WithExecutionTimingDemo';
...
[Global configuration](/#global-configuration)) so wrapping a hot-path
function doesn't flood the console. `warnIfAbove` is never throttled - it's
meant to be rare.

If you're inside a component and want a stable, memoized callback that's
also measured, see [`useMeasuredCallback`](/hooks/useMeasuredCallback).

## Live demo

<WithExecutionTimingDemo />
